...
Here is the Git history for Delphox's Blockbench model:

https://gitlab.com/cable-mc/cobblemon/-/commits/main/common/src/main/resources/assets/cobblemon/bedrock/pokemon/models/0655_delphox?ref_type=heads

Old model = nose is $2 \times 2$. New model = nose is $1 \times 1$.
You can check by importing the `.geo.json` file into Blockbench
or inspect the json for a `muzzle3` bone and its `cubes[0].size` array
(either `[1, 1, 1]` or `[2, 2, 2]`).

I want to use the older model cuz it looks cuter and less aggressive.

*Despite these two pokemon having similar commit histories,*
the old Delphox model was actually dropped before the Braxien one!

| Commit | File stem (in `blockbench/`) | Notes                                                                                                                     |
|---|------------------------------|---------------------------------------------------------------------------------------------------------------------------|
| `c466d5e0cbd39592fe0d1ed7dfeae5e2a2e76813` | `delphox`                    | Initial commit of Delphox, old model. No battle anim and always holds stick.  |
| `30b769db132a81b83dc6b1d61fa6b21e2b8dcdfc` | `delphox-2`                  | Nose is now $1 \times 1$  |
| `34315d9f1dc90f3109e0e21aa2942b18cccbeb3d` | N/A                          | CCPL licenses added to model folder. Previously, MPL2.0 applied to the models. Not sure if CCPL is retroactively applied. |

**Verdict:** use the Delphox assets from commit `c466d5e0cbd39592fe0d1ed7dfeae5e2a2e76813`
However, Delphox feels like it has incomplete animations without code-driven animations (compared to Braixen, whose animations are fully powered by Blockbench files).
As such, I will be shelving this Delphox port for now. Maybe someone else will finish it...

> Note: this repo contains the `30b769db132a81b83dc6b1d61fa6b21e2b8dcdfc` version of Delphox because I thought the old Delphox and Braixen models
> would share the same commit, so I blindly ported the wrong version without checking beforehand!
