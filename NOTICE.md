# Licensing Notice

_I'm not a lawyer! I just don't want to get DMCA'd!_

This project includes code and assets adapted from other mods, such as Cobblemon.
I've tried my best to document where I've used other people's licensed works
so that I won't get sued.
If you think I missed something, please submit a pull request on Gitlab and I would be
happy to correct the licencing.

Any file that is not explicitly licensed shall be assumed to be mine
(i.e. `Copyright (C) 2026 Riley`)
and licensed under the same license found in the repo's root.

## Cobblemon

Cobblemon's source code is licensed under the Mozilla Public License 2.0.
Cobblemon's Pokemon assets are licensed under CCPL (a custom CC license?).

Source code: https://gitlab.com/cable-mc/cobblemon/-/tree/main?ref_type=heads

MPL 2.0 License: https://www.mozilla.org/en-US/MPL/2.0/

CCPL: pasted verbatim in the folder where it is applicable

Assets and sources used:
- a few Pokemon assets (CCPL)
- `BedrockGeoModel.java` is heavily adapted from source code (MPL2.0)

---

_Any references listed above may be stale as the mod continues to evolve._
_Files may get moved around, and features may be added or removed._
_Please submit a PR on Gitlab if you need this file needs updated for legal reasons._
